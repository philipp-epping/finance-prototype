...
import { Sidebar, BankConnectionModal, BankAccountsTable, TransactionsTable, Dashboard, AccountDetailPanel, CategorizationFlow, ReceiptMatchingFlow, UploadDocumentsModal } from './components'
...
function App() {
  ...
  const renderContent = () => {
    switch (selectedNav) {
      ...
      case 'categorization':
      case 'review':
        return <CategorizationFlow onBack={() => handleNavChange('dashboard')} />
      case 'receipts':
  ...
  const renderAccountsContent = () => {
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="bg-white border-b border-fill-quaternary">
          <div className="px-3 py-2.5 flex items-center justify-between">
            <div className="px-1">
              <h1 className="text-14 font-medium text-text-primary">Bank Accounts</h1>
            </div>
            <Button size="sm" icon={Plus} onClick={() => setIsModalOpen(true)}>
              {bankAccounts.length === 0 ? 'Connect bank' : 'Add account'}
            </Button>
          </div>
        </div>
        ...
        {bankAccounts.length === 0 ? (
          <div className="flex-1 flex items-center justify-center">
            <EmptyState
              ...
              description="Connect your first bank account to start tracking your transactions."
              action={<Button onClick={() => setIsModalOpen(true)}>Connect your first bank</Button>}
            />
          </div>
        ) : (
          <div className="flex-1 overflow-auto p-6">
            ...
            {deactivatedAccounts.length > 0 && (
              <button
                ...
                className="mt-4 text-13 text-text-tertiary hover:text-text-secondary transition-colors"
              >
                {deactivatedText}
              </button>
            )}
